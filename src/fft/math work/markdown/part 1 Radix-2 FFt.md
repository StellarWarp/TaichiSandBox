...
进行一次FFT共需要 $L=\log_2N$ 层并行运算，每一层都是将 $N$ 个点分成两组，然后将这两组点合并成一个 $N/2$ 点的DFT
...
（其实这里有越界访问的问题，但因为在傅里叶变换中，数据是可以周期延拓的，只要在最后进行取模运算即可，为推导方便，这里不做取模运算）
...
&= \sum_{n=0}^{N-1}(x[n] + jy[n])\omega_N^{nk}\\
&= \sum_{n=0}^{N-1}x[n]\omega_N^{nk} + j\sum_{n=0}^{N-1}y[n]\omega_N^{nk}\\
&= X[k] + jY[k]\\
...
z[m,n] &= x[m,n] + jy[m,n]\\
Z[k,l] &= DFT(z[m,n]) = \sum_{m=0}^{R-1}\sum_{n=0}^{N-1}z[m,n]\omega_R^{mk}\omega_N^{nl}\\
&= \sum_{m=0}^{R-1}\sum_{n=0}^{N-1}(x[m,n] + jy[m,n])\omega_R^{mk}\omega_N^{nl}\\
&= \sum_{m=0}^{R-1}\sum_{n=0}^{N-1}x[m,n]\omega_R^{mk}\omega_N^{nl} + j\sum_{m=0}^{R-1}\sum_{n=0}^{N-1}y[m,n]\omega_R^{mk}\omega_N^{nl}\\
&= X[k,l] + jY[k,l]\\
X[k,l] &= \frac{1}{2}(Z[k,l] + Z^*[R-k,N-l])\\
Y[k,l] &= -\frac{1}{2j}(Z[k,l] - Z^*[R-k,N-l])\\
\end{align*}
$$
