...
def fft_pass(size: ti.template(),
    ...
    log2_min_radix = log2_size % log2_max_radix
    min_radix = 1 << log2_min_radix
    max_radix_pass = log2_size//log2_max_radix
    min_radix_pass = log2_size//log2_min_radix
    # max_min_radix_pass_diff = max_radix_pass - min_radix_pass
    pass_count = max_radix_pass + 1
    if log2_min_radix == 0:
        log2_min_radix = log2_max_radix
        min_radix = max_radix
        pass_count -= 1
    print(f'\nmax_radix = {max_radix}\n'
    ...
    for k in range(pixel_size):
        target = target_
        radix = max_radix
        log2_radix = log2_max_radix
        l = ti.u32(1)
        for step in range(1, pass_count+1):
            if step == pass_count:
                radix = min_radix
                log2_radix = log2_min_radix
                l = min_radix_pass
            else:
                l = step
            if k == 0: print(f'l = {l},radix = {radix}')
            logM_size = log2_size//log2_radix
            log2_b = log2_radix * (logM_size - l)
            b = 1 << log2_b
            w0 = fast_mul(fast_div(k, log2_b), log2_b)
            c = fast_mul(w0,log2_radix) - w0 + k
            # c = fast_mul(k, log2_radix) - fast_mod(k, b)
            # multiple target offset
            offset = fast_mul(fast_div(k, log2_size), log2_size)
            i = fast_mod(c, size) + offset
            # if k == 0:
            #     print('------------------')
            #     print(f'l = {l},radix = {radix} b = {b},log2_b = {log2_b} i = {i}, c = {c}, w0 = {w0}')
            #debug remove
        ...
        # for m in range(4):
        #     i = offset + m
        #     x = buffer[not target, i]
        #     phi = -2 * tm.pi / 4 * m * (k%size)
        #     twiddle = tm.vec2(tm.cos(phi), tm.sin(phi))
        #     if inverse:
        #         twiddle.y = -twiddle.y
        #     buffer[target, k] += tm.vec4(
        #         tm.cmul(x.xy, twiddle),
        #         tm.cmul(x.zw, twiddle))
        # target = not target
        #     # print(f'k = {k}, m = {m}, i = {i}')
    return pass_count
...
def fft_2d(xy: ti.template(), buffer: ti.template(), inverse: ti.template()):
    ...
    # horizontal
    swap_count = fft_pass(x_size, log2_x_size, pixel_size, log2_pixel_size,
    ...
    # vertical
    swap_count = fft_pass(y_size, log2_y_size, pixel_size, log2_pixel_size,
             inverse, buffer, target)
